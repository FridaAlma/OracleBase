...
### 1.3 Memory
`coding_agent.db` auto-stores history + agentic memory (last 3 runs injected).
...
### 1.10 NavPredictor
`workspace/nav_predictor/` — Decoder-only Transformer (1.4M params) predicting navigation events. SafetyDetector: 96.9% danger detection. CLI: `python -m workspace.nav_predictor.main`.

### 1.11 WikiTool
`tools/wiki_tool.py` — Manages Uninet wiki (192.168.1.5:8000): list, read/write HTML, upload images, search.
...
Workspace root: D:\Work\AGI\Oracle. Main files: coding_agent.py, cli.py, system_prompt.md, .env, .env.example, coding_agent.db, chat.html, oracle.bat, CONSTITUTION.md.